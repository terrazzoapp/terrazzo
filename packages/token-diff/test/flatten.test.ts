...
import type { DiffEntry, DiffListingEntry } from '../src/types.js';

describe('flattenDiffEntry', () => {
  describe('with DiffListingEntry', () => {
    const sampleEntry: DiffListingEntry<ListedToken> = {
      changeType: 'modified',
      name: 'color.primary',
      platform: 'web',
      mode: 'light',
    ...
    const createDiffListingEntry = (old: any, new_: any): DiffListingEntry<ListedToken> => ({
      changeType: 'modified',
      name: 'test.token',
      platform: 'web',
      mode: 'light',
    ...
    describe('nested props edge cases', () => {
      ...
      it('should contain the same props on both flattened tokens', () => {
        const entry = createDiffListingEntry(shallowToken, deepToken);
      ...
      it('should handle completely missing objects by creating null properties', () => {
        ...
        const entry = createDiffListingEntry(oldToken, newToken);
        const flattened = flattenDiffEntry(entry);
    ...
    describe('input edge cases', () => {
      it('should escape dots in property names when flattening', () => {
        ...
        const entry = createDiffListingEntry(token, token);
        const flattened = flattenDiffEntry(entry);
      ...
      it('should handle empty objects', () => {
        const entry = createDiffListingEntry({}, {});
        const flattened = flattenDiffEntry(entry);
      ...
      it('should handle null values', () => {
        const entry = createDiffListingEntry(null, null);
        const flattened = flattenDiffEntry(entry);
      ...
      it('should handle null old tokens', () => {
        ...
        const entry = createDiffListingEntry(null, token);
        const flattened = flattenDiffEntry(entry);
      ...
      it('should handle null new tokens', () => {
        ...
        const entry = createDiffListingEntry(token, null);
        const flattened = flattenDiffEntry(entry);
  ...
  describe('with DiffEntry', () => {
    const sampleEntry: DiffEntry<any> = {
      changeType: 'modified',
      old: {
        $name: 'color.primary',
        $type: 'color',
    ...
    const createDiffEntry = (old: any, new_: any): DiffEntry<any> => ({
      changeType: 'modified',
      old,
      new: new_,
    });
    ...
    describe('general functionality', () => {
      it('should preserve changeType', () => {
        ...
        expect(flattened).not.toHaveProperty('platform');
        expect(flattened).not.toHaveProperty('mode');
      });
      ...
      it('should flatten token props', () => {
        ...
        expect(flattened.old).toMatchObject({
          ...
          '$value.hex': '#cc99ff',
          $description: 'Primary color',
          '$extensions.custom\\.extension.metadata.category': 'primary',
          '$extensions.custom\\.extension.config': 'value',
        });
      ...
      it('should not contain intermerdiary props in flattened tokens', () => {
        ...
        expect(flattened.old).not.toHaveProperty('$extensions');
        expect(flattened.new).not.toHaveProperty('$extensions');
        expect(flattened.old).not.toHaveProperty('$extensions.custom\\.extension');
        expect(flattened.new).not.toHaveProperty('$extensions.custom\\.extension');
      });

      it('should handle nested objects in flattening', () => {
        const flattened = flattenDiffEntry(sampleEntry);

        expect(flattened.old).toHaveProperty('$extensions.custom\\.extension.metadata.category', 'primary');
        expect(flattened.old).toHaveProperty('$extensions.custom\\.extension.config', 'value');
        expect(flattened.old).not.toHaveProperty('$extensions.custom');
        expect(flattened.old).not.toHaveProperty('$extensions.custom.extension');
        expect(flattened.old).not.toHaveProperty('$extensions.custom\\.extension.metadata');
      });
    });
    ...
    describe('input edge cases', () => {
      ...
      it('should handle null old tokens', () => {
        ...
        expect(flattened.old).not.toBeNull();
        expect(flattened.old).toHaveProperty('$type', null);
        expect(flattened.old).toHaveProperty('$value.hex', null);
        expect(flattened.new).not.toBeNull();
        expect(flattened.new).toHaveProperty('$type', 'color');
        expect(flattened.new).toHaveProperty('$value.hex', '#cc99ff');
      });
      ...
      it('should handle null new tokens', () => {
        ...
        expect(flattened.old).not.toBeNull();
        expect(flattened.old).toHaveProperty('$type', 'color');
        expect(flattened.old).toHaveProperty('$value.hex', '#cc99ff');
        expect(flattened.new).not.toBeNull();
        expect(flattened.new).toHaveProperty('$type', null);
        expect(flattened.new).toHaveProperty('$value.hex', null);
      });
