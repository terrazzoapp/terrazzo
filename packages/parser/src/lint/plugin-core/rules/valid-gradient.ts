...
function validateGradient(
  value: unknown,
  {
    ...
    report: LintRuleContext<
      typeof ERROR_MISSING | typeof ERROR_POSITION | typeof ERROR_INVALID_PROP
    >['report'];
  },
) {
  if (Array.isArray(value)) {
    for (let i = 0; i < value.length; i++) {
      ...
      for (const key of Object.keys(stop)) {
        if (
          !GRADIENT_REQUIRED_STOP_PROPERTIES.includes(
            key as (typeof GRADIENT_REQUIRED_STOP_PROPERTIES)[number],
          )
        ) {
          report({
