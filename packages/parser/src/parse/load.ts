import { type DocumentNode, evaluate, type ObjectNode, print, type ValueNode } from '@humanwhocodes/momoa';
  ...
  for (const source of sources) {
    ...
    traverse(mutatedCopy, {
      ...
      enter(node, _, path) {
        ...
        const options = { source, sources: sharedCache, visited: [], aliasChain };
        try {
          switch (node.type) {
            case 'Object': {
            ...
            case 'Array': {
              for (let i = 0; i < node.elements.length; i++) {
                if (node.elements[i]!.value.type === 'Object') {
                  node.elements[i]!.value = flatten$refs(node.elements[i]!.value as ObjectNode, options) as ValueNode;
                }
              }
              break;
            }
            case 'String': {
              if (isAlias(node.value)) {
                const flattened = flatten$refs(aliasToMomoa(node.value)!, options);
                replaceNode(node, flattened);
              }
              break;
            }
      ...
      exit(node, _parent, path) {
        ...
        if (token) {
          tokens[token.jsonID] = merge(tokens[token.jsonID] ?? {}, token);
          // for $value, make sure any $refs aren’t deep-merged
          for (const mode of Object.keys(tokens[token.jsonID]!.mode)) {
            tokens[token.jsonID]!.mode[mode]!.$value = token.mode[mode]!.$value;
            if (mode === '.') {
              tokens[token.jsonID]!.$value = token.$value;
            }
          }
        }
      },
    ...
    traverse(source.document, {
      enter(node, _parent, path) {
        if (node.type === 'Object' && getObjMember(node, '$value')) {
          const id = `#/${path.join('/')}`;
          if (!tokens[id]) {
            logger.error({
              ...entry,
              message: 'Terrazzo encountered an error. was an error in parsing. Please open an issue.',
              node,
              src,
            });
          }
