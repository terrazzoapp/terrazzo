  ...
  it.each(tests)('%s', (_, want) => {
    ...
    for (const [jsonID, token] of Object.entries(want)) {
      tokens[jsonID] = {
        ...
        $type: token.$type,
        $value: (TOKEN_DATA as any)[token.$type].$value,
        aliasOf: undefined,
        partialAliasOf: undefined,
        aliasedBy: undefined,
        aliasChain: undefined,
        source: { node: parse(JSON.stringify((TOKEN_DATA as any)[token.$type])).body },
        dependencies: undefined,
      } as unknown as TokenNormalized;
    }

    const logger = new Logger();
    graphAliases(refMap, { tokens, sources: {}, logger });

    for (const [jsonID, graph] of Object.entries(want)) {
      expect(tokens[jsonID]).toEqual(expect.objectContaining(graph));
    }
  });
});
