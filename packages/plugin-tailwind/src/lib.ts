...
/** Metadata namespace this plugin stores descriptions under */
export const META_KEY = 'plugin-tailwind';

/**
 * Get a token’s description. Both this plugin and @terrazzo/plugin-css store the
 * description of the _resolved_ token in metadata, which is more accurate than
 * the base token for aliases and resolver permutations. Fall back to the base
 * token otherwise.
 */
export function getTokenDescription(token: TokenTransformed): string | undefined {
  const meta = token.meta as Record<string, { description?: string } | undefined> | undefined;
  return (
    meta?.[META_KEY]?.description ?? meta?.['plugin-css']?.description ?? token.token.$description
  );
}

/**
 * Format a token description as a CSS comment. Any `*\/` in the description is
 * escaped so a description can’t terminate the comment early and break the
 * generated CSS.
 */
export function formatDescriptionComment(description: string, indent = ''): string {
...
/**
 * For composite (multi-value) tokens, the sub-value that maps to the base
 * theme variable, per Tailwind conventions. e.g. a typography token mapped to
 * `text.tiny` generates `--text-tiny` from its font-size, while every other
 * sub-value uses Tailwind’s double-dash convention (`--text-tiny--line-height`).
 * @see https://tailwindcss.com/docs/font-size#customizing-your-theme
 */
export const BASE_SUB_VALUES: Record<string, string> = {
  typography: 'font-size',
};

/**
 * Format a composite (multi-value) token as Tailwind theme declarations,
 * using Tailwind’s double-dash convention for sub-values:
 *
 * ```css
 * --text-tiny: 0.625rem;
 ...
 * ```
 */
export function formatCompositeDeclarations(
  localID: string,
  value: Record<string, string>,
  $type: string,
): string[] {
  const baseSubValue = BASE_SUB_VALUES[$type];
  const declarations: string[] = [];
  if (baseSubValue && value[baseSubValue]) {
    declarations.push(`${localID}: ${value[baseSubValue]};`);
  }
  for (const [name, subValue] of Object.entries(value)) {
    if (name === baseSubValue) {
      continue;
    }
    declarations.push(`${localID}--${name}: ${subValue};`);
  }
  return declarations;
}
